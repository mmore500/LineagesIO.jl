...
- Verification artifact that must fail the bad implementation or fake-fix shape: a dispatch regression proving the sampled node-type parent-collection path resolves to the core method in `src/construction.jl`, not the extension override; direct canonical-owner versus wrapper behavior parity on rooted-network and tree-compatible rooted cases; and a narrow supplementary audit that `ext/PhyloNetworksIO.jl` no longer owns parent-collection type reconstruction through `Any[]`, `Vector{Any}`, `reduce(typejoin, ...)`, or any equivalent renamed helper. Current code fails the dispatch proof and the anti-regrowth audit.
...
- The work is not complete if the extension cleanup breaks rooted-network validation, extension activation behavior, retained annotation behavior, authoritative table retention, synthesized leaf-name handling, `MetaGraphsNext` weight and metadata semantics, or extension-owned traversal and downstream mutation behavior.
...
- Read the relevant tests in `test/core/canonical_load_owner.jl`, `test/core/fileio_load_surfaces.jl`, `test/core/alife_format.jl`, `test/extensions/metagraphsnext_activation.jl`, `test/extensions/metagraphsnext_simple_newick.jl`, `test/extensions/metagraphsnext_supplied_basenode.jl`, `test/extensions/metagraphsnext_tables_after_load.jl`, `test/extensions/metagraphsnext_abstracttrees.jl`, `test/extensions/metagraphsnext_network_rejection.jl`, `test/extensions/phylonetworks_activation.jl`, `test/extensions/phylonetworks_newick_networks.jl`, `test/extensions/phylonetworks_tree_compatible_newick.jl`, `test/extensions/phylonetworks_annotation_paths.jl`, `test/extensions/phylonetworks_tables_after_load.jl`, `test/extensions/phylonetworks_rejection_paths.jl`, `test/integration/phylonetworks_soft_release.jl`, and `test/runtests.jl` in full.
...
- `PhyloNetworks` and `MetaGraphsNext` extension activation behavior before and after weakdep load must remain intact.
...
- Pair that dispatch regression with a narrow supplementary audit of `ext/PhyloNetworksIO.jl` proving the extension no longer owns parent-collection type reconstruction through `Any[]`, `Vector{Any}`, `reduce(typejoin, ...)`, or any equivalent renamed helper. This audit may supplement but must not replace behavior-level parity.
...
- Keep the existing `MetaGraphsNext` and `PhyloNetworks` extension activation suites, `test/extensions/phylonetworks_annotation_paths.jl`, and both `*_tables_after_load.jl` suites active so extension activation, retained annotation, and authoritative-table retention remain explicit preserved behaviors rather than incidental fallout from the final full-suite gate.
...
**Verification**: Add a regression that proves the sampled `HybridNetwork` node-type parent-collection dispatch resolves to the core `src/construction.jl` method instead of the extension override. That regression must fail the current implementation. Pair it with a targeted post-edit audit of `ext/PhyloNetworksIO.jl` confirming there is no remaining parent-collection type reconstruction via `Any[]`, `Vector{Any}`, `reduce(typejoin, ...)`, or an equivalent renamed helper. Then keep `test/extensions/phylonetworks_activation.jl`, `test/extensions/phylonetworks_newick_networks.jl`, `test/extensions/phylonetworks_tree_compatible_newick.jl`, `test/extensions/phylonetworks_annotation_paths.jl`, `test/extensions/phylonetworks_tables_after_load.jl`, `test/extensions/phylonetworks_rejection_paths.jl`, and `test/integration/phylonetworks_soft_release.jl` green, and run the full test and docs gates.
...
**Files**: new `test/extensions/phylonetworks_canonical_owner.jl`, `test/extensions/phylonetworks_activation.jl`, `test/extensions/phylonetworks_newick_networks.jl`, `test/extensions/phylonetworks_tree_compatible_newick.jl`, `test/extensions/phylonetworks_annotation_paths.jl`, `test/extensions/phylonetworks_tables_after_load.jl`, `test/extensions/phylonetworks_rejection_paths.jl`, `test/integration/phylonetworks_soft_release.jl`, `test/runtests.jl`  
...
**Verification**: Add a regression that proves a supplied-instance sampled parent collection dispatches directly to the `MetaGraphsNextBuildCursor` multi-parent overload. Add a negative check that no graph-vector shim method survives after cleanup. Current code must fail the no-shim proof. Keep the existing activation, library-created tree path, supplied-instance network path, authoritative-table retention, rejection-path, and AbstractTrees view tests green, then run the full test and docs gates.
