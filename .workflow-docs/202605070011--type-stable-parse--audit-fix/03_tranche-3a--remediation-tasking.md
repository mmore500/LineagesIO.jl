...
> **Branch Narrow (2026-05-09):** The "later rooted-network edge" atomicity claim in this task was not fully delivered. Pre-materializing edge payloads (Task 1) guarantees first-edge atomicity: if the first user-owned constructor fails before any node is written, the supplied graph remains empty and the same object is retryable. However, a later-edge constructor failure (after nodes and earlier edges are already written) may leave partial graph state. The live test in `metagraphsnext_canonical_owner.jl` documents `nv == 4, ne == 3` after a later-edge throw, not `nv == 0, ne == 0`. The settled contract is Branch Narrow: first-edge atomicity is guaranteed; later-edge failure may leave partial state; callers who require retry safety after any failure must discard the partially-populated graph and supply a fresh empty instance. The function for the supplied-instance path is now `read_lineages!` (not `read_lineages`), making the mutation explicit at the call site.  
...
> **Branch Narrow (2026-05-09):** The "nv == 0, ne == 0 after later-edge failure" assertion in this task was not delivered for later-edge throws. The first-edge case (missing vertex or edge constructor on the very first edge) does guarantee `nv == 0, ne == 0` and same-target retry. The later-edge case on a rooted-network fixture leaves partial graph state. The live `metagraphsnext_public_surface.jl` test documents `nv == 0, ne == 0` for the missing-edge constructor only because that particular test uses a fixture whose first edge triggers the throw. The settled contract is Branch Narrow (see Task 2 annotation). The supplied-instance function is now `read_lineages!`.  
