...
function fileio_load(file::FileIO.File{AlifeStandardFormat}, args...; builder = nothing, kwargs...)::LineageGraphStore
    assert_supported_alife_load_keywords(kwargs)
    open(file, "r") do stream
        return fileio_load(stream, args...; builder = builder)
    end
end
...
function fileio_load(stream::FileIO.Stream{AlifeStandardFormat}, args...; builder = nothing, kwargs...)::LineageGraphStore
    assert_supported_alife_load_keywords(kwargs)
    load_request = build_alife_load_request(args, builder)
    source_path = normalize_source_path(FileIO.filename(stream))
    source_text = read(FileIO.stream(stream), String)
...
function assert_supported_alife_load_keywords(kwargs)::Nothing
    isempty(kwargs) || throw(ArgumentError("Unsupported keyword options for alife standard loads. Supported surfaces are `load(src)`, `load(src, NodeT)`, `load(src, basenode)`, and `load(src; builder = fn)`."))
    return nothing
end

function build_alife_load_request(args::Tuple{}, builder)::AbstractLoadRequest
    builder === nothing && return TablesOnlyLoadRequest()
    return BuilderLoadRequest(builder)
end

function build_alife_load_request(args::Tuple{Type}, builder)::AbstractLoadRequest
...
function build_alife_load_request(args::Tuple{BasenodeT}, builder)::AbstractLoadRequest where {BasenodeT}
...
function build_alife_load_request(args::Tuple, builder)::AbstractLoadRequest
    throw(ArgumentError("Alife standard loads accept at most one positional construction target. Supported surfaces are `load(src)`, `load(src, NodeT)`, `load(src, basenode)`, and `load(src; builder = fn)`."))
end
